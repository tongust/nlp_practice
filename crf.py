...
class LinearChainCRF():
    ...
    def _get_training_feature_data1(self):
        ret = list()
        for X,_ in self.training_data:
            ret0 = list()
            for t in range(len(X)):
                feature_list = self.feature_set.get_feature_list(X, t)
                ret0.append(feature_list)
            ret.append(ret0)
        return ret
    ...
    def _estimate_parameters(self):
        ...
        * References:
        ...
        """
        training_feature_data = self._get_training_feature_data1()
